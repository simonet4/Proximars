...
// Variables
  ...
  int displayMode1 = 0; // Mode d'affichage initial : 0 pour température, 1 pour humidité air, 2 pour humidité sol, 3 pour luminosité
...
void loop() {
  ...
  // Activation du bouton pour l'intervalle d'affichage automatique
    if (etatBouton == HIGH && etatPrecedentBouton == LOW) {
        displayMode1 = (displayMode1 + 1) % 4; // Changer de mode d'affichage à chaque appui sur le bouton  
    ...
    if (tempsEcoule >= Inactivite) { // Vérifier si le bouton a été inactif pendant 10 secondes
      if (maintenant - BasculeAutoBouton >= IntervalleBasculeAutoBouton) { //vérifier si 3 secondes se sont écoulées depuis la dernière bascule automatique          
        displayMode1 = (displayMode1 + 1) % 4; // Changer de mode d'affichage          
