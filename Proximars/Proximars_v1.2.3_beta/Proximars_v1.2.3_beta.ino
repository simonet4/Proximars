...
// Modifications: Correction de l'envoi des données via le bluetooth
// Détails: Rangé et commenté, un arduino méga est nécéssaire pour l'envoi des données bluetooth
...
// Broches
  // Digital
    #define boutonPin A4 // Bouton sur la broche 2
    #define bulleurPin 2 // Bulleur sur la broche 3
    #define arrosagePin 3 // Arrosage sur la broche 6
    #define eclairagePin 4 // éclairage sur la broche 7
    #define chauffagePin 5 // chauffage sur la broche 8
    #define ventilateurPin 6 // ventilateur sur la broche 9
    SoftwareSerial BTserial(15, 14); // RX sur 15, TX sur 14 pour le module Bluetooth
  //Analog
    DHT dht(A0, DHT22);
  //I2C
    ...
    // Contrôle des composants
      ...
      // Contrôle de l'éclairage en fonction de la luminosité et de l'heure
        // récupération de l'heure
          if (BTserial.available() > 0) { // Vérifiez s'il y a des données Bluetooth disponibles
            int hour = BTserial.read(); // Lire les données reçues
          
        if (hour <= 8 && hour >= 20 && luminosite < 100) {
